...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function create() {
      ...
      for (var i = 0; i < 35; i++) {
        days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="lunar-day"></div><div class="marker"></div></div>';
      }
      //添加下拉框数据
        ...
        if (data) {
          daysElement.forEach(function (v, i) {
            ...
            if (data[year + '-' + currentmonth + '-' + day]) {
              ...
              daysElement[i].querySelector('.marker').innerHTML = data[year + '-' + currentmonth + '-' + day];
            } else {
              v.classList.remove('sc-mark');
              v.title = '';
              daysElement[i].querySelector('.marker').innerHTML = '';
            }
          });
        }
      }
